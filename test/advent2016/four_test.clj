...
(deftest count-letters-test
  (testing "count-letters"
    (println "count-letter totallyrealroom")
    (println (count-letters "totallyrealroom"))))
    ; (is (= (count-letters
    ;         (first (tokenize input1)))))))

(deftest tokenize-test
  (testing "tokenize"
    (is (= (tokenize input1) ["aaaaabbbzyx" 123 "abxyz"]))
    (is (= (tokenize input2) ["abcdefgh" 987 "abcde"]))
    (is (= (tokenize input3) ["notarealroom" 404 "oarel"]))
    (is (= (tokenize input4) ["totallyrealroom" 200 "decoy"]))))

(deftest sort-count-map-test
  (testing "sort-count-map"
    (println "sort-count-map")
    (println (sort-count-map {2 [\c \b \a], 1 [\e \d \f], 3 [\l \m]}))))
...
(def input "hqcfqwydw-fbqijys-whqii-huiuqhsx-660[qhiwf]
...
(deftest decrypt-letter-test
  (testing "decrypt-letter"
    (println "decrypt-letter")
    (is (= (decrypt-letter \a 1) \b))
    (is (= (decrypt-letter \z 1) \a))
    (is (= (decrypt-letter \a 26) \a))
    (is (= (decrypt-letter \q 343) \v))
    (is (= (decrypt-letter \z 343) \e))
    (is (= (decrypt-letter \m 343) \r))
    (is (= (decrypt-letter \t 343) \y))))

(deftest tokenize2-test
  (testing "tokenize2"
    (is (= (tokenize2 "qzmt-zixmtkozy-ivhz-343") ["qzmt zixmtkozy ivhz" 343]))))

(deftest decrypt-test
  (testing "decrypt"
    (println (decrypt "qzmt-zixmtkozy-ivhz-343"))))
